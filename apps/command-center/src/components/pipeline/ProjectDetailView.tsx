    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-6 mb-6">
        ...
        <div className="flex items-center gap-3 w-[420px] flex-shrink-0">
          {/* Project Icon */}
          {project.icon ? (
            // Check if it's an emoji (starts with an emoji character) or an image path
            /^[\p{Emoji_Presentation}\p{Extended_Pictographic}]/u.test(project.icon) ? (
              // Emoji icon
              <div className="w-9 h-9 bg-zinc-800 rounded-lg flex items-center justify-center shadow-md flex-shrink-0">
                <span className="text-xl">{project.icon}</span>
              </div>
            ) : (
              // Image path icon
              <img
                src={getAssetUrl(`dev://${project.repoPath}/assets/${project.icon}`)}
                alt={`${project.name} icon`}
                className="w-9 h-9 rounded-lg shadow-md flex-shrink-0 object-cover"
                onError={(e) => {
          ...
          {/* Default icon (shown when no icon or image load fails) */}
          <div className={clsx(
            'w-9 h-9 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center shadow-md shadow-blue-500/20 flex-shrink-0',
            project.icon && 'hidden'
          )}>
      ...
      {/* Bottom Section: Documents, Mockups, Tech Stack, Details */}
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <div>
            ...
            <div className="space-y-1.5 max-h-[160px] overflow-y-auto">
              {[
                { name: 'Pipeline (Dark)', file: 'pipeline-view-v4.html' },
                { name: 'Pipeline (Light)', file: 'pipeline-view-v4-light.html' },
                { name: 'Mobile (Dark)', file: 'mobile-view-v4.html' },
                { name: 'Mobile (Light)', file: 'mobile-view-v4-light.html' },
                { name: 'Tabbed Layout', file: 'command-center-tabbed.html' },
                { name: 'Color Palette', file: 'color-palette-mockups.html' },
              ].map((mockup) => (
                <a
                  key={mockup.file}
                  href={`/mockups/${mockup.file}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 p-1.5 rounded-lg bg-zinc-800/50 hover:bg-zinc-700/50 transition-colors group"
                >
                  <svg className="w-3 h-3 text-zinc-500 group-hover:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                  </svg>
                  <span className="text-xs text-zinc-400 group-hover:text-zinc-200">{mockup.name}</span>
                </a>
              ))}
            </div>
          </div>
              ...
              {project.techStack.map((tech) => {
                ...
                return (
                  <div key={tech} className="relative group">
                    <span className="px-2 py-1.5 bg-zinc-800 rounded-lg text-xs font-medium text-zinc-300 border border-zinc-700 cursor-help block text-center hover:border-zinc-500 transition-colors truncate">
                      {tech}
                    </span>
                    {/* Tooltip */}
                    {info && (
            ...
            <div className="space-y-1.5 text-xs mb-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-zinc-200">
                  {project.startedAt ? new Date(project.startedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-zinc-200">{project.complexity === 'F' ? 'Full' : 'Easy'}</span>
              </div>
            </div>
            <div className="space-y-2">
              <button
                onClick={handleOpenVSCode}
                className="w-full px-3 py-2 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-xl text-xs font-medium text-zinc-300 transition-colors flex items-center justify-center gap-1.5"
              ...
              <button
                ...
                className="w-full px-3 py-2 bg-blue-600 hover:bg-blue-500 rounded-xl text-xs font-medium text-white transition-colors shadow-lg shadow-blue-500/20 flex items-center justify-center gap-1.5"
